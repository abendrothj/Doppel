// Reporting and output for BOLA-Fuzz
...
pub fn export_csv(results: &[(String, String, String)]) {
    let mut file = File::create("bola_fuzz_report.csv").unwrap();
...
pub fn export_markdown(results: &[(String, String, String)]) {
    let mut file = File::create("bola_fuzz_report.md").unwrap();
    writeln!(file, "# BOLA-Fuzz Report\n").unwrap();
    for (method, url, verdict) in results {
        writeln!(file, "- **{}** {}: {}", method, url, verdict).unwrap();
    }
}
